 ...
 * @return string
 */
function theme_aicda_get_main_scss_content($theme) {
    global $CFG;
    ...
    // Aicda scss.
    $aicdavariables = file_get_contents($CFG->dirroot . '/theme/aicda/scss/aicda/_variables.scss');
    $aicda = file_get_contents($CFG->dirroot . '/theme/aicda/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/aicda/scss/aicda/_security.scss');

    $lastpreset = '';
    if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_aicda', 'preset', 0, '/', $filename))) {
        $lastpreset = $presetfile->get_content();
    }

    // Combine them together.
    $allscss = $aicdavariables . "\n" . $scss . "\n" . $aicda . "\n" . $lastpreset .    "\n" . $security;
 ...
 * @return string
 */
function theme_aicda_get_extra_scss($theme) {
    $content = '';
    ...
    if (empty($loginbgimgurl)) {
        $loginbgimgurl = new \moodle_url('/theme/aicda/pix/loginbg.png');
        $loginbgimgurl->out();
    }
 ...
 * @return string
 */
function theme_aicda_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_aicda_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/aicda/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_aicda_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('aicda');
